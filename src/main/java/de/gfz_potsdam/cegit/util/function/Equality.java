 ...
 */
package de.gfz_potsdam.cegit.util.function;
 ...
 * Implementing classes should ensure that:</p>
 * <ul>
 * <li> The {@link #compare compare} function is consistent with
 * {@link #equal(Object, Object) equal}. If two objects {@link #compare compare}
 * to {@code 0} then they are {@link #equal(Object, Object) equal} and the
 * reciprocal is true (this ensures that sorted collections/maps do not break
 * the general contract of their parent class based on object equal).</li>
 * <li> The {@link #hashOf hash} function is consistent with
 * {@link #equal(Object, Object) equal}: If two objects are equal, they have the
 * same hashcode (the reciprocal is not true).</li>
...
public interface Equality<T> extends EqualityComparer<T>, Comparator<T>, Serializable {
     ...
     * {@link #equal(Object, Object)}). Two objects considered equal have the
     * same hash code. The hash code of <code>null</code> is always
     * <code>0</code>.
     *
    ...
    /**
     * Indicates if the specified objects can be considered equal. This methods
     * is equivalent to {@code (compare(o1, o2) == 0)} but usually faster.
     *
